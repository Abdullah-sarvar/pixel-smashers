    ...
    <div class="bg-[var(--card)] border border-[var(--border)] rounded-xl p-10 w-full max-w-md pixel-border relative">
        ...
        <form method="POST" action="/login">
            ...
            <div class="mb-6">
                ...
                <input type="password" name="password" placeholder="••••••••" required class="w-full py-3 px-4 bg-[var(--bg)] border border-[var(--border)] rounded-md text-[var(--text)] text-[15px] focus:outline-none focus:border-[var(--accent)] transition-colors pixel-border-sm">
            </div>
            <button type="submit" class="btn-pixel w-full !text-sm">LOGIN</button>
        </form>

        <div class="text-center mt-6 text-sm text-[var(--muted)]">
            Don't have an account? <a href="/register" class="text-[var(--accent-light)] no-underline font-semibold hover:underline">Sign Up</a>
        </div>
    </div>
</div>
@endsection